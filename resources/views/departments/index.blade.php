...
@section('title', 'Departments - Task Management System')
@section('page-title', 'Departments')

@section('content')
<div class="mb-6 flex justify-between items-center">
    <h3 class="text-2xl font-bold text-gray-800">📁 All Departments</h3>
    <a href="{{ route('departments.create') }}" class="btn-primary text-white px-6 py-3 rounded-lg font-semibold hover:shadow-lg transition">
        ➕ Add New Department
    </a>
</div>
        ...
        <tbody>
            @forelse($departments as $department)
                <tr class="border-b hover:bg-blue-50 transition duration-200">
                    <td class="px-6 py-4 text-sm">
                        <div class="flex items-center gap-3">
                            <span class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-blue-100 text-blue-600 font-bold">
                                {{ substr($department->DepartmentName, 0, 1) }}
                            </span>
                            <div>
                                <p class="font-semibold text-gray-800">{{ $department->DepartmentName }}</p>
                                <p class="text-xs text-gray-500">ID: {{ $department->DepartmentID }}</p>
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-4 text-sm">
                    ...
                    <td class="px-6 py-4 text-sm text-gray-600">{{ $department->created_at->format('M d, Y') }}</td>
                    <td class="px-6 py-4 text-center">
                        <div class="flex justify-center gap-2">
                            <a href="{{ route('departments.show', $department) }}" class="inline-flex items-center px-3 py-1 rounded-lg bg-blue-100 text-blue-600 hover:bg-blue-200 transition font-semibold text-xs">
                                👁️ View
                            </a>
                            <a href="{{ route('departments.edit', $department) }}" class="inline-flex items-center px-3 py-1 rounded-lg bg-yellow-100 text-yellow-600 hover:bg-yellow-200 transition font-semibold text-xs">
                                ✏️ Edit
                            </a>
            ...
            @empty
                <tr>
                    <td colspan="5" class="px-6 py-8 text-center text-gray-500">
                        📭 No departments found. <a href="{{ route('departments.create') }}" class="text-blue-500 hover:underline font-semibold">Create one</a>
                    </td>
                </tr>
            @endforelse
        </tbody>
